...
import { Send, CheckCircle2, AlertCircle, ChevronRight, ChevronLeft, Mail, MessageSquare, Info, LifeBuoy, CreditCard, MessageCircle, Bug } from "lucide-react";
...
export default function SupportPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSubmitted(true);
        // Assuming the backend returns something we can use to find the ticket
        // Or we show a generic success first. Actually the user wants ticket ID
        // The mock backend returns status: success. I'll need a real ID later.
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-text-muted mb-8 leading-relaxed">
            Your support request has been logged. ARIA is analyzing it right now.
          </p>
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-700 mb-8 font-mono text-sm">
             <span className="text-text-muted">Ticket ID:</span> <span className="text-emerald-500">{ticketId}</span>
          </div>
          <div className="flex flex-col gap-3">
            ...
            >
             ...
             <button 
              onClick={() => { setStep(1); setSubmitted(false); setFormData({ ...formData, subject: "", message: "" }); }}
    ...
    <div className="min-h-screen py-20 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-12 text-center">
          <Link href="/" className="inline-block mb-6 text-text-muted hover:text-white transition-colors">
            ← Back to Home
          </Link>
        ...
        {/* Progress Bar */}
        <div className="w-full h-1 bg-slate-800 rounded-full mb-12 flex">
           <motion.div 
            initial={{ width: "33.3%" }}
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
               ...
               >
                 ...
                 <div className="space-y-4">
                   <div className="relative group">
                     <label className="text-xs font-bold uppercase tracking-widest text-text-muted group-focus-within:text-emerald-500 transition-colors">Name</label>
                     <input 
                      type="text"
                      value={formData.name}
                   ...
                   <div className="relative group">
                     <label className="text-xs font-bold uppercase tracking-widest text-text-muted group-focus-within:text-emerald-500 transition-colors">Email Address</label>
                     <div className="relative">
                      <input 
                        ...
                        className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500 font-medium transition-all"
                      />
                      {formData.email.includes("@") && <CheckCircle2 className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500 w-5 h-5 animate-in fade-in zoom-in" />}
                     </div>
                   </div>
                 </div>
               </motion.div>
             ...
             {step === 2 && (
               ...
               >
                 ...
                 <div className="space-y-6">
                        ...
                        {categories.map((cat) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center p-4 rounded-xl border transition-all ${isActive ? 'bg-emerald-500/10 border-emerald-500 text-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'bg-slate-950 border-slate-700 text-text-muted hover:border-slate-500'}`}
                            >
                    ...
                    <div className="space-y-4">
                       <label className="text-xs font-bold uppercase tracking-widest text-text-muted">Priority</label>
                          ...
                          {priorities.map((p) => {
                             ...
                             return (
                               <button
                                ...
                                className={`flex-1 p-4 rounded-xl border text-center font-bold relative transition-all ${p.color} ${isActive ? 'ring-2 ring-inset ring-current scale-[1.02]' : 'opacity-60 grayscale'}`}
                               >
                                {p.label}
                               </button>
                             );
                 ...
                 <div className="space-y-4">
                    <div className="relative">
                      <textarea 
                        ...
                        placeholder={
                          formData.category === "bug_report" ? "Describe the bug, steps to reproduce, and expected vs actual behavior..." : 
                          formData.category === "billing" ? "What's wrong with your subscription or payment?" :
                          "Explain your request in detail..."
                        }
                        rows={8}
                        className="w-full bg-slate-950 border border-slate-700 rounded-2xl p-6 focus:outline-none focus:border-emerald-500 font-medium transition-all"
                      />
           ...
           <div className="mt-10 flex justify-between items-center gap-4">
              ...
              {step < 3 ? (
                <button 
                  onClick={nextStep}
                  className="flex items-center px-8 py-4 bg-white text-slate-950 hover:bg-slate-100 rounded-xl transition-all font-bold group ml-auto"
                >
                ...
                <button 
                  ...
                  disabled={loading}
                  className="flex items-center px-10 py-4 bg-emerald-500 hover:bg-emerald-600 text-slate-950 rounded-xl transition-all font-bold shadow-[0_0_20px_rgba(16,185,129,0.4)] disabled:opacity-50 disabled:grayscale ml-auto"
                >
                  {loading ? "Sending..." : "Submit Request"} <Send className="w-5 h-5 ml-2" />
                </button>
              )}
           </div>
        </div>
        ...
        <div className="mt-12 p-6 bg-slate-900/50 border border-slate-800 rounded-2xl flex items-start gap-4">
           ...
           <p className="text-sm text-text-muted leading-relaxed">
             This form uses ARIA (Autonomous Response & Intelligence Agent). Responses are generated instantly after analysis. For urgent matters, please ensure priority is set to High.
           </p>
        </div>
      </div>
    </div>
